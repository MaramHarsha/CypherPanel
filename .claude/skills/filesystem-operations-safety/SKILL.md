...
> **Status: code-grounded (Phase 4, shipped).** The File Manager is implemented in `internal/filemanager`: `CleanRel` neutralises traversal (unit-tested), `safePath` does a symlink-resolved under-root re-check, and every op runs **as the account uid/gid** via `setfsuid`/`setfsgid` on a locked OS thread (refuses uid 0). Transport is NATS request-reply (`Subject`); Core proxies via `internal/api/filemanager_handler.go`. FTP (`internal/ftp`) maps virtual users to the account uid/gid + home. **Now also implemented:** the `extract` op unpacks zips with per-entry **zip-slip validation** (CleanRel under-root re-check), symlink rejection, and zip-bomb size caps; **byte-quota enforcement** on writes/extracts (package `disk_mb`, agent computes tree size). Still to add: **inode-count** quota. This remains the highest-risk surface — a bug here is a cross-account breach. Read [[linux-system-integration]].
