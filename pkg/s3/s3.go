// Package s3 is a minimal S3-compatible client: AWS Signature Version 4 over
// net/http, with no SDK.
//
// It lives in pkg/ because BOTH planes write to S3 — the agent takes database
// and volume backups, and the control plane archives log lines — and two
// implementations of a signing algorithm is two places for a subtle bug that
// only shows up against one provider.
//
// Tiny footprint on purpose (vision non-negotiable 1): the AWS SDK is tens of
// megabytes of dependency for four HTTP verbs.
package s3
...
// Client speaks the four verbs this project needs.
type Client struct {
	client *http.Client
}
...
func (s *Client) Delete(
	...
	return s.request(ctx, http.MethodDelete, endpoint, bucket, region, key, accessKey, secretKey, nil, 0)
}

// Download downloads data from S3 using GET.
func (s *Client) Download(
	ctx context.Context,
	endpoint, bucket, region, key, accessKey, secretKey string,
) (io.ReadCloser, error) {
...
func (s *Client) buildURL(endpoint, bucket, key string) (*url.URL, error) {
...
func (s *Client) sign(req *http.Request, region, accessKey, secretKey string, payloadHash *string) {
	now := time.Now().UTC()
