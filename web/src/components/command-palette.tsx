...
export function CommandPalette() {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent title="Command palette" className="top-[20%] max-w-lg translate-y-0 p-0">
        ...
        <ul ref={listRef} className="max-h-80 overflow-y-auto p-1.5" role="listbox">
          {filtered.length === 0 ? (
            <li className="px-3 py-6 text-center text-[13px] text-text-faint">No matches</li>
          ) : (
            filtered.map((item, i) => (
              <li key={item.id}>
                <button
                  ...
                  onClick={() => run(item)}
                  className={cn(
                    "flex w-full items-center justify-between gap-2 rounded px-2.5 py-2 text-left text-[13px]",
                    i === active ? "bg-raised text-text" : "text-text-mid",
                  )}
                >
                  <span className="flex min-w-0 items-center gap-2.5">
                    <span className="shrink-0 text-text-faint">{item.icon}</span>
                    <span className="truncate">{item.label}</span>
                  </span>
                  <span className="mono flex items-center gap-2 text-[11px] text-text-faint">
                    {item.hint}
