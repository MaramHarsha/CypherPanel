...
export const renameTeam = async (id: string,
  ...
  return apiFetch<Team>(getRenameTeamUrl(id),
  {
    ...
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json', ...options?.headers },
    body: JSON.stringify(renameTeamRequest)
  }
);}
...
const {query: queryOptions, request: requestOptions} = options ?? {};
  ...
  const queryKey =  queryOptions?.queryKey ?? getListTeamMembersQueryKey(id);
   ...
   return  { queryKey, queryFn, enabled: id !== null && id !== undefined, ...queryOptions} as UseQueryOptions<Awaited<ReturnType<typeof listTeamMembers>>, TError, TData> & { queryKey: DataTag<QueryKey, TData, TError> }
}
...
export const useChangeTeamMemberRole = <TError = BadRequestResponse | UnauthorizedResponse | ForbiddenResponse | NotFoundResponse | Error,
 ...
 , queryClient?: QueryClient): UseMutationResult<
        ...
        TContext
      > => {
      return useMutation(getChangeTeamMemberRoleMutationOptions(options), queryClient);
    }
