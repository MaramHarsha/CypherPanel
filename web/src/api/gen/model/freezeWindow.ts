...
export interface FreezeWindow {
  ...
  /**
     ...
     */
  end_minute: number;
  /** IANA name, e.g. "Europe/Berlin". The window is evaluated on WALL CLOCK in this zone, so a DST change makes it an hour shorter or longer in absolute terms twice a year — which is what "no deploys after six on Friday" means. The window is half-open and may wrap the week (Fri 18:00 → Mon 08:00 is one window). */
  timezone: string;
  /** The window rendered as the sentence the panel and the CLI both print, e.g. "Fri 18:00 → Mon 08:00 (Europe/Berlin)". Derived, never stored. */
  summary: string;
}
