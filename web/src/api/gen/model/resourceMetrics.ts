 ...
 */
import type { ResourceDisk } from './resourceDisk.ts';
import type { ResourceMetricPoint } from './resourceMetricPoint.ts';
import type { ResourceMetricSummary } from './resourceMetricSummary.ts';

export interface ResourceMetrics {
  resolution: string;
  /**
     * The newest bucket, so a chart four minutes stale can say so.
     * @nullable
     */
  as_of?: string | null;
  /** False when the panel has collection off. A resource with no data then reads collection is off, not idle. */
  collecting: boolean;
