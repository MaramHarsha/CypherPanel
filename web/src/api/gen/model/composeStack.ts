 ...
 */
import type { ComposeRoute } from './composeRoute.ts';
import type { ComposeStackStatus } from './composeStackStatus.ts';
...
export interface ComposeStack {
  ...
  /** @maxLength 100 */
  name: string;
  /** The server this stack runs on. */
  server_id: string;
  route: ComposeRoute;
  /**
     * The revision the agent is converging toward; null before the first deploy.
     * @nullable
     */
  desired_revision_id?: string | null;
  /** Observed (ADR-005), in the same vocabulary an Application uses. `degraded` is some but not all of the services compose was asked for; `stopped` is the birth state. */
  status: ComposeStackStatus;
  /** Compose's own last words on failure. Never contains a variable's value. */
  status_detail?: string;
  /** The revision the agent last reported actually running. */
