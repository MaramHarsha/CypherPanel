 ...
 */
import type { PublicStatusComponentState } from './publicStatusComponentState.ts';
import type { PublicStatusDay } from './publicStatusDay.ts';

export interface PublicStatusComponent {
  label: string;
  state: PublicStatusComponentState;
  /**
     * Percentage over OBSERVED time, or null when nothing was observed. Null renders as "—", never as 100% — printing a perfect score for a service that has measured nothing is the most common dishonesty in this product category.
     * @nullable
     */
  uptime?: number | null;
