...
import type { CreateComposeStackRequestEnvVars } from './createComposeStackRequestEnvVars.ts';

export interface CreateComposeStackRequest {
  /** @maxLength 100 */
  name: string;
  /** A server that runs workloads; a builder-role server is a 400. */
  server_id: string;
  /**
     * The compose file. Must parse as YAML and declare at least one service. `build:` and `container_name:` are refused; everything else runs as-is.
     * @maxLength 524288
     */
  compose_yaml: string;
  route?: ComposeRoute;
  /** Sealed on the way in and never returned. Compose interpolates `${KEY}` from them, so a secret need not live in the file. */
  env_vars?: CreateComposeStackRequestEnvVars;
}
