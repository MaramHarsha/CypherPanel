 ...
 */
import type { LogDrainRequestConfig } from './logDrainRequestConfig.ts';
import type { LogDrainRequestKind } from './logDrainRequestKind.ts';

export interface LogDrainRequest {
  name?: string;
  /** Set on create; the kind is a drain's identity and is not patchable. */
  kind?: LogDrainRequestKind;
  /** Empty means every project. */
  project_id?: string;
  target_id?: string;
  /** The destination's own config, sealed whole. Loki takes `url`, optional `headers` and optional `labels`; syslog takes `address` and an optional `app_name`; s3 takes an optional `prefix` and gets its credentials from the Backup Target. Omit it entirely and send only `enabled` to pause a drain without re-sending the credential. */
  config?: LogDrainRequestConfig;
  enabled?: boolean;
}
