...
export interface SetPanelTLSRequest {
  /** A plain email address (`ops@example.com`), not a display-name form — the CA registers the address itself. Empty clears the account and turns certificate issuance off fleet-wide. */
  acme_email: string;
  /** Optional absolute http(s) ACME directory URL. Empty means Let's Encrypt production. */
