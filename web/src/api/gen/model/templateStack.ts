 ...
 */
import type { TemplateStackEnv } from './templateStackEnv.ts';
import type { TemplateStackRoute } from './templateStackRoute.ts';

export interface TemplateStack {
  name: string;
  /** The compose file, with this catalog's placeholders unresolved. */
  compose: string;
  /** Which service answers, and on which port. Absent means the stack publishes nothing. */
  route?: TemplateStackRoute;
  /** Values sealed on install and delivered through the stack's env file. Generated ones are `{{secret.N}}` here; the compose file references them as `${VAR}` and never contains the value. */
  env?: TemplateStackEnv;
}
