 ...
 */
import type { CreateNotifierRequestChannel } from './createNotifierRequestChannel.ts';
import type { CreateNotifierRequestConfig } from './createNotifierRequestConfig.ts';
import type { CreateNotifierRequestEventsItem } from './createNotifierRequestEventsItem.ts';

export interface CreateNotifierRequest {
  name: string;
  channel: CreateNotifierRequestChannel;
  /** Channel-specific config, sealed at rest and never returned. email: {smtp_host, smtp_port, username, password, from, to}; discord/slack: {webhook_url}; telegram: {bot_token, chat_id}. */
  config: CreateNotifierRequestConfig;
  /** @minItems 1 */
  events: CreateNotifierRequestEventsItem[];
  enabled?: boolean;
}
