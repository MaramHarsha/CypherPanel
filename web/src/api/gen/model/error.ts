...
export interface Error {
  ...
  error: string;
  /** The value of this response's `X-Request-Id` header, repeated in the body so a screenshot of a failure carries it. Paste it into a bug report or `GET /api/v1/panel/logs` to find the request. Present on every response served through the API's middleware. */
  trace_id?: string;
  /**
     * Present only on `429`: whole seconds until the throttle lifts, the same value as the `Retry-After` header. Count down against it rather than guessing.
     * @minimum 1
     */
