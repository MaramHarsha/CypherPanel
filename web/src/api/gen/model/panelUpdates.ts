 ...
 */
import type { PanelUpdatesKind } from './panelUpdatesKind.ts';
import type { PanelUpdatesMode } from './panelUpdatesMode.ts';
import type { PanelUpgrade } from './panelUpgrade.ts';

export interface PanelUpdates {
  current: string;
  latest?: string;
  kind?: PanelUpdatesKind;
  notes_url?: string;
  /** `manual` is a container install, which cannot upgrade itself and hands over the two commands instead. */
  mode: PanelUpdatesMode;
  active?: PanelUpgrade | null;
}
