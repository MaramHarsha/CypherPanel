 ...
 */
import type { AgentChannel } from './agentChannel.ts';
import type { AgentServer } from './agentServer.ts';
import type { AgentUpdatesRunningVersions } from './agentUpdatesRunningVersions.ts';

export interface AgentUpdates {
  /** The plane's own build, which is the ceiling on what may be asked of an agent. */
  panel_version: string;
  channels: AgentChannel[];
  servers: AgentServer[];
  /** Running version to host count - the fleet histogram the empty state shows. */
  running_versions: AgentUpdatesRunningVersions;
}
