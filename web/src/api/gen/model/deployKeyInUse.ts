 ...
 */
import type { DeployKeyInUseApplicationsItem } from './deployKeyInUseApplicationsItem.ts';

/**
 * The `Error` envelope plus the applications blocking a deploy-key delete. Additive: a client that reads only `error` is unaffected.
 */
export interface DeployKeyInUse {
  error: string;
  trace_id?: string;
  /** The applications that still reference the key. */
  applications: DeployKeyInUseApplicationsItem[];
}
