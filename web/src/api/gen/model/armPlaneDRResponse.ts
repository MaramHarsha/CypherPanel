 ...
 */
import type { PlaneDisasterRecovery } from './planeDisasterRecovery.ts';

export interface ArmPlaneDRResponse {
  config: PlaneDisasterRecovery;
  /** Present exactly once, in the response to the request that generated it. It is the only thing that can open a snapshot, and it is exactly as powerful as the panel's master key — because the master key is inside. */
  recovery_key?: string;
}
