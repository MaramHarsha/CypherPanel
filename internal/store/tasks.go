...
func (s *Tasks) Create(ctx context.Context, serverID, taskType string, payload json.RawMessage, createdBy string) (*Task, error) {
	if payload == nil {
		payload = json.RawMessage("{}")
	}
	row := s.pool.QueryRow(ctx, `
		INSERT INTO tasks (server_id, type, payload, created_by)
		VALUES ($1, $2, $3, NULLIF($4, '')::uuid)
		RETURNING id, server_id, type, payload, status, error, COALESCE(created_by::text, ''), created_at, updated_at`,
		serverID, taskType, payload, createdBy)
	return scanTask(row)
}

func (s *Tasks) GetByID(ctx context.Context, id string) (*Task, error) {
	row := s.pool.QueryRow(ctx, `
		SELECT id, server_id, type, payload, status, error, COALESCE(created_by::text, ''), created_at, updated_at
...
func scanTask(row pgx.Row) (*Task, error) {
	var t Task
	err := row.Scan(&t.ID, &t.ServerID, &t.Type, &t.Payload, &t.Status, &t.Error, &t.CreatedBy, &t.CreatedAt, &t.UpdatedAt)
