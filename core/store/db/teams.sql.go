...
SELECT id, email, password_hash, role, totp_secret_enc, totp_secret_nonce, created_at, updated_at, totp_enabled, display_name, timezone FROM users ORDER BY created_at
`
...
UPDATE users SET role = $2, updated_at = now() WHERE id = $1 RETURNING id, email, password_hash, role, totp_secret_enc, totp_secret_nonce, created_at, updated_at, totp_enabled, display_name, timezone
`
