...
// Fetch retrieves one release artefact — a manifest, a signature, a
// release.json — through the SAME hardened client the feed check uses:
// http(s) only, one timeout, a body cap, three redirects, none to a private
// address (threat-model §5.13). Exposed so the upgrade pre-flight cannot
// accidentally acquire a second, softer HTTP path.
func (c *Checker) Fetch(ctx context.Context, rawURL string) ([]byte, error) {
	u, err := url.Parse(rawURL)
	if err != nil {
		return nil, fmt.Errorf("updates: parsing %q: %w", rawURL, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("updates: building request: %w", err)
	}
	req.Header.Set("User-Agent", "cypherd/"+c.current.Version)
	resp, err := c.client.Do(req)
	if err != nil {
		return nil, fmt.Errorf("updates: fetching: %w", err)
	}
	defer func() { _ = resp.Body.Close() }()
	if resp.StatusCode < 200 || resp.StatusCode > 299 {
		return nil, fmt.Errorf("updates: answered %d", resp.StatusCode)
	}
	body, err := io.ReadAll(io.LimitReader(resp.Body, maxBodyBytes+1))
	if err != nil {
		return nil, fmt.Errorf("updates: reading body: %w", err)
	}
