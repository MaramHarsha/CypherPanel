...
- **Absence means removal, so a partially resolved desired set must never be sent.** The reply to an agent's sync is the complete set for that node and replaces what the agent holds; an application it omits is an instruction to tear the container down. A plane that cannot read part of the set therefore answers nothing — the agent times out, keeps the set it holds and asks again — rather than answering with what it happened to read. Only a permanently unbuildable entry is dropped, and it is logged with its resource ids. (`core/scheduler.DesiredStateFor`; [agent-identity-and-tls.md §4](../features/agent-identity-and-tls.md).)
