...
| Auto-update of the platform (safe-by-design) | ⚠️ (breaks: #3687, #7193, #7599) | ✅ | **V1.x** | The community's #1 trust wound in Coolify — bricked panels, lost encryption keys. Split across two features, because [ADR-010](../adrs/ADR-010-agent-auto-update.md) §6 is explicit that **the plane never updates itself**: ADR-010 covers the AGENT (desired version, channels, health-gated convergence — [agent-updates.md](../features/agent-updates.md)), while the operator-driven panel upgrade with its pre-flight snapshot, atomic apply, health-verified rollback and update lock is [panel-updates.md](../features/panel-updates.md); see [research/community-pain-points.md](../../research/community-pain-points.md) |
