...
- **Release Channel** — one of two named rows (`stable`, `canary`) holding a **Desired Agent Version**. A Server follows a channel rather than naming a version, so promoting a release is one write instead of a bulk edit, and `canary` is opt-in per Server. Both channels ship empty, and empty means *no instruction* — never "downgrade to nothing".
- **Desired Agent Version** — the agent binary a Server should be running, resolved from the channel it follows and carried in its desired state like any other spec. The plane names it and never stores or serves a byte of the binary: the agent fetches a signed artifact over its own outbound HTTPS and runs nothing a key baked into the running binary does not verify.
- **Maintenance Mode** — an Application's declaration that its front door is closed on purpose: the Proxy serves a generic holding page (`503`, `Retry-After`) in place of the app while the app itself keeps running, keeps passing its health check and can still be deployed. It is *current* Application state, never a revision snapshot, so a rollback cannot lift it, and it has no expiry — a window that lifted itself mid-migration would publish a half-migrated Application. Distinct from `stopped`, which is a resource not running, and from `degraded`, which is a resource serving with something wrong: maintenance mode is downtime on purpose and is never a status word.
- **Preview Password** — one passphrase per Application that gates its **Preview Environments** only, asked for by the Proxy before anything is served. Stored as a bcrypt hash and readable exactly once, at the moment it is set. It never applies to a standing Environment: a flag left on must not lock production behind a passphrase nobody remembers setting.
- **IP Allowlist** — the set of networks that may reach an Application through the Proxy, expressed as normalized CIDRs. Current Application state like Maintenance Mode, enabled separately from its contents, and refused when enabled with nothing in it — an allowlist that allows nobody is a maintenance page with worse copy.
