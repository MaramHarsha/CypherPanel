...
**cypher-builder** — a role flag on the agent, not a separate program. **Today** (the Phase 2 slice): each docker agent builds its own applications — builder = target, the ADR-008 no-relay case — via the local Docker daemon's build API, so the image is already where it runs. **Planned** (with multi-server distribution): the `--role=builder` split routes builds to dedicated builder agents, streamed to targets via mTLS relay through the plane ([ADR-008](adrs/ADR-008-no-registry-required.md) — no registry required either way, external registries optional); that split is what will keep production workers build-free by default, fixing Dokploy's manager-starvation problem. Railpack/Nixpacks auto-detected builds and Compose parsing are later matrix rows. One constraint holds in every phase: **builds never run on the control plane.**
