...
**cypher-agent** — a single static Go binary (~20MB) per server. Outbound-only connection to the control plane (mTLS over gRPC/NATS) — meaning **no inbound ports, no SSH keys stored on the panel**, works behind NAT ([ADR-002](adrs/ADR-002-agent-dial-home-no-ssh.md)). Responsibilities: reconcile containers against desired state via the local Docker socket, write Traefik dynamic config, stream logs/metrics/events upward, run health checks locally. Orchestrator *drivers* share one desired-state schema with different reconcilers: `docker` (standalone) is implemented and the only launch driver ([ADR-006](adrs/ADR-006-docker-only-at-launch.md)); `swarm` is a committed V1.x fast-follow and `k8s` a post-v1 direction — planned, not present.

**cypher-builder** — a role flag on the agent, not a separate program ([builder-role-and-relay.md](features/builder-role-and-relay.md)). `cypher-agent run --role=all|builder|worker`: an `all` agent (the single-server default) builds its own applications via the local Docker daemon's build API — builder = target, the ADR-008 no-relay case, the image already where it runs. With a `worker`-role target, the scheduler routes the build to a `builder`-role agent and the image streams builder → plane → target as a transient mTLS gRPC relay ([ADR-008](adrs/ADR-008-no-registry-required.md) — bounded buffers, never on plane disk, no registry required either way; external registries optional). The split keeps production workers build-free by default, fixing Dokploy's manager-starvation problem. Railpack/Nixpacks auto-detected builds and Compose parsing are later matrix rows. One constraint holds in every phase: **builds never run on the control plane.**
