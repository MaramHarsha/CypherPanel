...
**Deploy:** git webhook → Core API resolves app + branch rules → Scheduler enqueues `work.build` → builder clones, builds, pushes image, streams logs over `logs.build.*` → on success Scheduler flips desired state to new image → target agent pulls, starts new container, health-checks it, rewrites Traefik config, drains old container (zero-downtime by default) → agent publishes `state.deploy.succeeded`.
