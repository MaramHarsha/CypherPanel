...
- **The first-run claim is gated by a code only the host's console has seen, and it is atomic.** `POST /api/v1/auth/setup` is the panel's one unauthenticated route that creates an *owner*, and `install.sh` opens the panel's port to the internet the moment it finishes — so "reach your own box first" is a race the operator can lose to a scanner. The installer generates `CYPHERD_SETUP_TOKEN`, keeps it in `cypherd.env` (root, 0600) and prints it; when it is configured the claim needs it (constant-time compare, 403 otherwise, never logged). The count-and-create runs under a transaction-scoped advisory lock, so two claims that arrive together cannot both count zero users. A panel without a code — a dev panel, `docker compose`, the env-var bootstrap — claims as before. `[V1: first-run-setup.md, implementation note]`
- **Public routes, enumerated.** Without a session or an agent certificate a caller reaches exactly: `GET/POST /api/v1/auth/setup` (above), `POST /api/v1/auth/login` (throttled, §5.8), `GET /api/v1/ca.pem` (the plane's public CA, pinned by the installer), `GET /install/agent.sh`, the two invitation routes (above), the email-change confirm route (§5.10), the GitHub webhook and the per-application push webhook (both signature-verified), and the static assets. Everything else is `authed` or `sessionOnly`. `[V1]`
